 ...
 */
 ...
 public class Main {
    ...
    public static void main(String[] args) {
        ...
        User adminUser = new User(true); /*defining variables with admin and not admin roles */

        PolicyCheckRequestHandler policyCheckHandler = new PolicyCheckRequestHandler(null);
        ExistingContentCheckRequestHandler existingContentHandler = new ExistingContentCheckRequestHandler(policyCheckHandler);
        RenderContentHandler renderContentHandler = new RenderContentHandler(existingContentHandler);
        
        WebServer server = new WebServer(renderContentHandler);

        //test the server and get response in the console : 
        WebRequest request = new WebRequest("/home", regularUser);
        String response = server.getRequest(request);
        System.out.println(response);
        

        //test the server and get responses in logs.txt. We will also print everything in the console
        FileLogger fileLogger = new FileLogger("/Users/hugocaetano/Desktop/src/com/company/logs.txt"); /* 2-b) */
        server.registerObserver(fileLogger);

        /**
         * Expected output :
         * Status 403 : user is not authorized to access this content
         */
        server.getRequest(new WebRequest("/dashboard", regularUser));
         ...
         * Status 404 : Page missing
         */
        server.getRequest(new WebRequest("/dashboard/nonExistingPage", adminUser));

        /**
         * Expected output :
         * Status 200 : Dashboard content here
         */
        server.getRequest(new WebRequest("/dashboard", adminUser));

        /**
         * Expected output :
         * Status 200 : Home content here
         */
        server.getRequest(new WebRequest("/home", regularUser));
